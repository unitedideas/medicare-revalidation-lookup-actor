...
- Primary conversion: start a paid Actor run for 1 to 100 NPIs.
- Conversion-quality metric: a successful run that returns one current, source-dated result per valid NPI without claiming live PECOS status.
...
## Current same-category evidence
...
- Decision path: exact input limit, result fields, price unit, source date, and limits before secondary monitoring handoff.
...
- Conversion hypothesis: exact revalidation language and predictable per-NPI pricing will outperform a generic CMS scraper for high-intent credentialing searches.
